...
- Razor surface to be projected: 53 page models (~10.8k LOC) under
  ...
  request; `Contracts/Vocabulary/OperatorVocabulary.cs` is the single
  code → operator-vocabulary map, reached by the Core-typed
  `Presentation/OperatorLabels.cs` adapter consumed by 24 `.cshtml` files.
...
| Operator vocabulary | GWY-016 moves the pure code → label map into `src/Pegasus.Contracts/Vocabulary/OperatorVocabulary.cs` (pure strings, no ASP.NET dependency), leaving `src/Pegasus.Web/Presentation/OperatorLabels.cs` as the Core-typed adapter so web and desktop share one list. **Deviation:** the proposal is silent; the repository's one-list-per-concept rule requires it. | AGENTS.md § Simplicity rails; GWY-016 |
...
| DSK-03-16 | `OperatorLabels` relocation to `Pegasus.Contracts` (one vocabulary list) with web consumers re-pointed (implemented by GWY-016) | chore | DSK-03-01 | Web signatures remain unchanged; desktop consumes the same map; no second copy; the two sanctioned intake terms follow the design authority | Base/post characterization; existing web tests green; architecture ownership guard | 1, 5 | pegasus-gateway-dev · dotnet-webapi · Kanmer |
