...
| 4 | Malicious or malformed attachment | Reader and envelope limits run before Core; the upload-security ticket owns safe paths and content validation. | `src/Pegasus.Core/Intake/IntakeContracts.cs:7-57`; `docs/current-architecture.md:222-236`; [[DSK-10-06]] | [[DSK-10-06]] | Novel parser defects and provider-specific content remain residual risk; owner: intake and security test maintainers. |
...
| Scan-time source-value rule for `src/Pegasus.Web/appsettings.json:12` | The scanner reads the exact JSON password value at scan time and matches it without copying the secret into this register; the rule is removed when [[DSK-10-02]] removes the account |
