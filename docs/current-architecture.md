...
shared `/connect/token` composition described above. Desktop password grants at `/connect/token` share the staff sign-in throttle with browser `POST /Account/SignIn` requests. This source-state entry
