...
edited across ~23 stale references (measured, see step 8 — this is more than the three
documents the body's Guardrails name). Eight untracked `winui-*` folders (18 files,
7.9 MiB) are removed from disk and produce **no git diff at all**, which is why the proof
for that half has to be `git status --porcelain` before and after rather than a diff.
...
3. **Prove the payload survived, not just the Markdown.** The local tree measured
   ...
   **7,911,936 bytes**), `ls .agents/skills/vendor/windows/winui-dev-workflow/analyzer/`
   ...
   — 18 files. The vendored tree should match, skill for skill.
...
8. **Fix every stale pointer. There are 23 of them across nine files, not three.**
   Measured 2026-08-24 with
   `grep -rln '\.codex/skills' --include='*.md' --include='*.toml' docs/ AGENTS.md .codex/ .agents/ .grok/`:

   | File | Hits | What it says today |
   | --- | --- | --- |
   | `docs/desktop/README.md` | 1 (`:101`) | Routing legend: WinUI skills "vendored under `.codex/skills/` today" |
   | `docs/desktop/12-agent-tooling/README.md` | 8 (`:43`, `:46-47`, `:53`, `:104`, `:115`, `:117-118`, `:170`) | § 2 evidence base, § 3 deviation, § 5 the DSK-12-04 row itself |
   ...
   Rewrite each to the vendored path (`.agents/skills/vendor/windows/<name>/`), and where a
   sentence records history (§ 2's evidence base, § 3's deviation, the `09` note about two
   copies) **keep it as a dated historical note** rather than erasing the fact. Re-run the
   grep afterwards and expect zero live routing instructions.
   **Scope note for the reviewer:** the body's Guardrails say "may … edit the three
   documents named above". Step 8 and the acceptance criterion "No document or agent TOML
   still points at `.codex/skills`" require all nine. The operative instruction wins;
   editing the other six area plans is in scope for this ticket and is a one-line path
   change in each. Nothing else in those files is touched.
9. **Check the agent TOMLs.** Verified 2026-08-24: **no `.codex/agents/*.toml` contains the
   string `.codex/skills`** — the eight TOMLs name skills by name and the project skill by
   its `.agents/skills/project/pegasus-desktop/SKILL.md` path only. Re-run the grep to
   confirm nothing changed, and make no edit if it has not.
...
12. **Record the Appendix C evidence**: the before/after `find` and
    `git status --porcelain` output, the step 4 hash comparison, the `/skills` listing, and
    the list of nine documents edited.
...
3. `grep -rn '\.codex/skills' --include='*.md' docs/` → only dated historical sentences, no
   live routing instruction. (Baseline before this ticket: 23 hits across nine files.)
4. `pwsh ./eng/skills/verify-skills.ps1` → exit 0 **after** the deletions.
5. `pwsh ./scripts/Test-DocumentationLinks.ps1` →
   `All relative Markdown links resolve (<n> files checked).`
6. The operator's `/skills` listing → each skill exactly once.
...
| The body's Guardrails scope boundary names three documents; the real fix touches nine. | Recorded in step 8 with the measured list so the reviewer sees a decision, not scope creep. |
...
Open questions: **none.** Every decision in this ticket is already settled by
`docs/desktop/12-agent-tooling/README.md` § 3 and § 7 and by [[TOOL-001]]'s verdict; no
