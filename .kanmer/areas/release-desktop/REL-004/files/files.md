...
Paths under `eng/packaging/` are created by [[REL-002]] (plan handle `DSK-09-02`) and
[[REL-003]] (plan handle `DSK-09-03`); `src/Pegasus.Desktop/` is created by
[[FND-030]] (plan handle `DSK-02-05`). `ls src/` today shows only `Pegasus.Core`,
`Pegasus.Infrastructure`, `Pegasus.Web`, `Pegasus.Worker`, and `ls eng` returns nothing.
...
| `eng/packaging/New-DesktopReleaseManifest.ps1` (created by [[REL-002]]) | The thirteen-field manifest contract and its parameter names — this script's call site at step 9. |
| `eng/packaging/New-AppInstaller.ps1` and `eng/packaging/Test-AppInstaller.ps1` (created by [[REL-003]]) | The `.appinstaller` generation and the eight-check validator whose **exit code** is this script's gate. The validator prints a pass/fail list and exits non-zero on any failure. |

## Ripple effects

- **Runbook R1 step 2** becomes a real command; [[REL-009]] (plan handle `DSK-09-11`) runs
  it for the first pilot release and its proof quotes this script's output.
- **[[REL-014]]** extends this script: it fills the `-SbomPath`
  ...
  extension is additive. Its step 1 also reads this ticket's `open-questions` document for
  the SBOM question left there, so that document is part of the interface between the two.
- **[[REL-015]] (plan handle `DSK-09-17`)** invokes this script with `-Sign -CertificatePath
...
- **[[REL-011]] (plan handle `DSK-09-13`)** relies on the fact that this script **rebuilds
...
- **No CI ripple from this ticket.** `scripts/Get-CiChangeFlags.ps1:11`'s `$buildPattern`
  ...
  script by name, so adding it changes no lane's trigger. [[REL-005]] (plan handle
  `DSK-09-05`) owns the CI side.
...
- **Publishing.** The script writes to `artifacts/desktop-releases/<ver>/` and stops.
  Copying to the feed is `eng/packaging/Publish-DesktopRelease.ps1`, owned by
  [[REL-008]] (plan handle `DSK-09-10`), and R1 puts an approval phrase between the two.
...
- **Choosing an SBOM generator.** [[REL-014]] is the single owner of the
  generator choice, the vulnerability-gate contract and the suppression register. This
  ticket adds `-SbomPath` and invents nothing. It is a scope boundary rather than a live
  question **and it is still recorded in `open-questions/`**, below
  `## Parked (explicitly deferred)` — the ticket body's step 8 instructs that document in as
  many words, and `REL-014`'s step 1 goes looking for "the open question `DSK-09-04` left".
  Parked, so it blocks no move; recorded, so the sibling finds it. An earlier draft of this
  section said no such document was created, on the false ground that an unticked item
  "would block every stage move" — it blocks `leave-preparing`, `enter-review` and
  `enter-done`, never `leave-backlog`, and a parked entry blocks none of them.
- **Signing on a PR build.** Never. Signing happens only with an explicit `-Sign` plus a
  certificate route, on the authorised terminal or in the protected tag job ([[REL-015]]).
