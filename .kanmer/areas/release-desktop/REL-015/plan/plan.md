...
coordinated with [[FND-009]] (plan handle `DSK-00-09`).
...
> minimum-version gate), authored by [[REL-001]] (plan handle `DSK-09-01`); see [[REL-001]]'s
> plan for the ownership reconciliation — ADR-0105 has three claimants (`REL-001`, `FND-005`,
> `FND-042`). Its Consequences record
...
- **C-01** — the repositories become private; a self-hosted runner on the share host is the
  cost-driven shape, decided by [[TEST-019]] (plan handle `DSK-08-19`).
...
- **Skills**, loaded in this order: `pegasus-desktop`
  ...
  `.agents/skills/authoring-github-workflows/SKILL.md` once
  [[TOOL-002]] (plan handle `DSK-12-02`) vendors it — it is **not** in `.agents/skills/`
  today) → `winui-packaging`
  (`.codex/skills/winui-packaging/SKILL.md`, `microsoft/win-dev-skills` v0.5.0 `f1028dd5`,
  verified present; the path moves to `.agents/skills/vendor/windows/winui-packaging/` once
  `TOOL-002` lands).
...
- **Kanmer pipeline** for profile `feature`: `kanmer-research` → `kanmer-plan` →
  ...
  For a `feature` the gated boundaries are `leave-backlog` (`governing-doc`),
  `leave-preparing` (`research`, `files`, `plan`, `checklist`, **`questions-resolved`**),
  `enter-review` (`post-implementation-report`, **`questions-resolved`**) and `enter-done`
  (`proof`, **`questions-resolved`**) — `questions-resolved` at three of the four, and never
  at `leave-backlog`.
...
4. **Run on the signing host's self-hosted runner.**
   `runs-on: [self-hosted, windows, pegasus-signing]` — taking the labels from [[TEST-019]]
   rather than inventing them, and adding `timeout-minutes` so a
...
5. **Add the two-part approval gate.** Put the signing/publishing job behind a repository
   ...
   `FEED PUBLISH GRANTED pilot <ver>` — in the wording [[REL-009]] (plan handle `DSK-09-11`)
   step 2 confirmed — in the ticket before approving. Both are required: **the phrase is the
   audit trail, the environment is the mechanism.** If the environment feature is unavailable
   on this repository, keep the job `if: false` and record that fact rather than publishing on
   the phrase alone.
...
10. **Dry run first, with a development certificate to a local feed.** Point
    `-CertificatePath` at the development certificate from [[REL-006]] (plan handle
    `DSK-09-06`) and `-FeedRoot` at the Test/UAT stack's share, and run the whole lane.
    **Only after that passes** should the real certificate and the real pilot feed be used.
    Record both runs.
...
12. **Update both runbooks in this task.** In `runbooks.md` § R1, note which steps this lane
    ...
    `desktop/v<M.m.b>` tag now triggers this lane, coordinating with [[FND-009]], which owns
    the tag convention there.
...
- **Risk — the signing host is a single point of failure and a high-value target.** Its
  isolation and permissions are [[TEST-019]]'s to specify and this lane's to respect; do not
  loosen them here.
- **Open questions: none opened, and the reason is not cost.** An earlier draft of this
  section said "opening one would block every stage move for a runner decision `DSK-08-19`
  owns". The first half is false and is withdrawn: an unticked `- [ ]` line above `## Parked`
  blocks exactly `leave-preparing`, `enter-review` and `enter-done`, never `leave-backlog`,
  and for this `feature` `questions-resolved` sits at three of four boundaries. Blocking
  Preparing would have been affordable if there were a real question.

  The second half is the real reason and it stands: the runner's existence, isolation,
  permissions and labels are decided by [[TEST-019]] — a **named sibling ticket**, so under
  the authoring contract it is a scope boundary recorded here, not an `open-questions` entry.
  The other two unknowns are answerable by looking rather than by asking: whether the
  self-hosted runner is registered, and whether the `environment:` feature is available on
  this repository. Each has a recorded fallback that leaves the workflow reviewable and
  mergeable — land the job `if: false` with a comment — and neither fallback may be traded for
  a hosted-signing route or for publishing on the recorded phrase alone. Nothing in this
  ticket's body instructs that a question be recorded in `open-questions/`.
