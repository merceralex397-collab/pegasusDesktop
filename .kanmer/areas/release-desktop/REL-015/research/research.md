...
- The fork has **no tags** (`git tag` empty on 2026-08-23, recorded in
  ...
  be automated once D-002 settles how packages are signed." `DSK-00-09` (board `FND-009`)
  owns recording that convention in `docs/engineering.md`.

**No parity-matrix row covers this.** `PAR-01`…`PAR-46` are Razor page models; a release
automation lane is repository infrastructure. Nothing here is parity work.
...
- **The approval is two things, not one.** The body requires both a repository
  ...
  wording is confirmed by `DSK-09-11` (board `REL-009`) step 2;
...
- **Publish order is package first, `.appinstaller` last, previous package retained** — R9
  steps 1–2, implemented by `eng/packaging/Publish-DesktopRelease.ps1` (`DSK-09-10`, board
  `REL-008`).
- **`--timestamp` is mandatory** — `.codex/skills/winui-packaging/SKILL.md` § Key Rules:
  "without it, signatures expire with the cert".
- **The runner strategy belongs to another ticket.** `DSK-08-19` (board `TEST-019`), "CI cost
  and runner plan for the private-repository era (C-01)", decides the runner's isolation and
  permissions; this lane respects that decision rather than making it.
...
| The fork has no tags; the convention is `gateway/r<N>` and `desktop/v<M.m.b>`, with signing on `main` tags only and production publish staying a runbook step | `docs/desktop/00-governance-and-workflow/README.md` § 2 and `:203-211`; `DSK-00-09` (board `FND-009`) owns recording it |
...
| The approval phrase is proposed and confirmed by `DSK-09-11` (board `REL-009`); `MERGE AUTH GRANTED` keeps one meaning | `runbooks.md` § Conventions |
...
- **A-09-33 — a self-hosted runner exists, or is planned, on the signing host.** Nothing in
  this repository configures one; `DSK-08-19` (board `TEST-019`) decides the runner strategy.
...
- **A-09-34 — the runner's labels are agreed.** The body proposes
  ...
  *Confirmed by*: the labels recorded by `DSK-08-19` (board `TEST-019`) or by the operator
...
| Unattended execution — must it run with every desktop closed? | **yes, in the trigger sense** | The lane fires on a tag push and runs without a human at the keyboard — but only **after** an environment approval, so it is semi-attended. It runs on the in-house signing host's self-hosted runner, **not** a cloud service. |
| Protected credentials — long-lived secret that must not sit on workstations? | **yes** | The signing `.pfx` is a ~3-year private key. D-002 forbids it as a GitHub secret and confines it to the signing host, which is precisely why this lane must run on a self-hosted runner there and why no `secrets.` reference may appear in the workflow. |
...
| Central enforcement — revocation, permissions, audit, invariant independent of the client? | **no** | The lane's own gates (tag provenance, `signtool verify`, the `.appinstaller` validator, the environment approval) are build-time controls. The client-facing fail-closed rule is the gateway minimum-version gate, `DSK-04-06` (board `GWY-023`). |
...
- **Dry run with the dev certificate first.** `DSK-09-06` (board `REL-006`) produced one, and
  the Test/UAT stack has a share. A dry run exercises every step except the one that cannot be
  undone.
...
- **Does the self-hosted runner exist?** (A-09-33, A-09-34.) `DSK-08-19` (board `TEST-019`)
  decides the runner strategy, its isolation and its permissions. It is **not blocking**: the
  body gives an explicit, recorded fallback — land the workflow with `if: false` and a
  comment — so the work can be completed, reviewed and merged before the runner exists. **No
  `open-questions` document is created**; opening one would block every stage move for a
  decision another ticket owns.
- **Is a GitHub `environment:` with a required reviewer available?** (A-09-36.) Established by
  creating it. If not, the job stays disabled and the fact is recorded.
