...
- [x] **U-1 — Raw handler enumeration is in the ticket scratch.** Run
...
- [x] **U-2 — Difference list (a) is written: page models with no `PAR` row.** Join the 53
...
- [x] **U-3 — Difference list (b) is written: `PAR` rows whose page model no longer exists.**
...
- [x] **U-4 — Difference list (c) is written: handlers in code missing from a row's handler
...
- [x] **U-5 — Every multi-command handler is expanded into its command set.**
...
- [x] **U-6 — The reconciled skeleton table is written into `research`.** One table of
...
- [x] **U-7 — A-01-1 is confirmed: no HTTP surface exists beyond the Razor pages and the four
...
- [x] **U-8 — `PAR-24`'s "13 commands" is settled as a miscount or as a removed command.** Run
...
- [x] **U-9 — Both documentation gates pass after the edits.**
...
- [x] **U-10 — The `README.md:50` base-class count correction is decided.** Research F-9b
...
- [x] **U-11 — The pathspec correction is handed to [[FND-016]].** Research F-5 measures
