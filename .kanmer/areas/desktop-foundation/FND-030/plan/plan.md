...
## Independent review correction 5 — 2026-08-28

- Simplified the WebView2 remediation: removed the custom MSBuild item/deletion targets and the now-redundant WebView2 properties. Added one centrally pinned Microsoft.Web.WebView2 1.0.3719.77 PackageReference with ExcludeAssets=all and PrivateAssets=all. This preserves the Windows App SDK restore graph while excluding compile/runtime/native/content/build assets from the scaffold output; the project contains no WebView2 control or renderer caller.
- Exact task head is d2fe4bd08f8e63c3655097e4f850a0f086072176. The exact direct project and full solution locked restores passed. Full Release solution build passed with 0 warnings and 0 errors. Architecture tests passed 111/111. git diff --check passed.
- BuildAndRun.ps1 -SkipRun passed with 0 warnings/errors. The real winapp run path launched package identity CollisionEngineers.Pegasus_e6z0b4cw4baw0 as PID 3576; the responding Pegasus.Desktop window was captured and closed cleanly. The durable screenshot is artifacts/fnd-030/desktop-launch-desktop.png with SHA-256 C7838380DE4EACF053DFB0C9F7969F529DFAB76B11A8E4D1F5E1D5970A9B6159 and is indexed by assets/desktop-launch.md.
- A post-launch scan of the generated AppX directory found no Microsoft.Web.WebView2.Core.dll, Microsoft.Web.WebView2.Core.Projection.dll, Microsoft.Web.WebView2.Core.winmd, or WebView2Loader.dll. The lock graph visibly retains the package as a direct asset-exclusion override because Windows App SDK WinUI declares it; no ADR-0108 renderer exception is claimed.
- Re-measured filtered self-contained Release output: 519 files and 234279635 bytes (223.45 MiB). This supersedes the pre-exclusion 522-file/224.99 MiB measurement.
