...
- [x] Add an explicit `Microsoft.WindowsAppSDK.Analyzers` `PackageReference` to `src/Pegasus.Desktop/Pegasus.Desktop.csproj` so the `WUI*` diagnostics survive a plain `dotnet build` (not just a `BuildAndRun.ps1` build).
