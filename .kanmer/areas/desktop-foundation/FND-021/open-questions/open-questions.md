...
- [ ] **U-1 · authenticated session and role.** **Operator step.** The session must be
      ...
      `subscription_list` and `group_list`. Evidence: the session is authenticated and the subscription id/tenant match, but the resolved role is Owner at subscription scope, not reader-level. No credential is stored in the repository. U-1 remains open because the least-privilege criterion is not satisfied; rerun under a Reader-scoped identity.
- [x] **U-2 · `group_resource_list` for `rg-pegasus-prod`, raw JSON saved to scratch.**
      This is the set every register row is compared against; without it steps 5, 8
      and 9 are guesses.
- [x] **U-3 · one saved show/list output per resource type**, each named after the tool
...
- [x] **U-4 · drift comparison, recorded and never removed.** Every resource in Azure
...
- [x] **U-5 · `allowBlobPublicAccess` on both storage accounts and the Log Analytics
...
- [x] **U-6 · a real `path:line` "Used by (code path)" for every register row.**
...
- [x] **U-7 · a proposal §19 target position for every row**, drawn only from that
      ...
      removal condition.
- [x] **U-8 · "Declared absent" re-confirmed with a dated check per entry** — all nine
...
- [ ] **U-9 · cost context recorded read-only** against the `pegasus-prod-monthly`
      ...
      forecast): current spend `29.50478827580924` GBP against `75.0` GBP is proven;
      the read exposed no forecast amount, so U-9 remains open pending a usable
      forecast read. No change is proposed.
- [x] **U-10 · the tagging approval text written and the ticket stopped there.** For
...
- [x] **U-11 · the register written back, the proof attached, the link check green.**
