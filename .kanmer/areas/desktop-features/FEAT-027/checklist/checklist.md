...
- [ ] Confirm the four read ports by reading them: `GetRequestOperations` (`RequestOperations.cs:72`), `GetEmailOperations` (`EmailOperations.cs:62`), `ListPollHealthAsync` (`RetainedMail.cs:382`) and `ListMailboxesAsync` (`:379`); record the `MailboxId` join and the fact that `GetEmailOperations` has no Razor caller today
- [ ] Add `IntakeStatusResponse` + `MailboxIntakeStatus` (`mailboxId`, `mailboxAddress`, `isPolled`, `lastCompletedAtUtc`, `lastFailureCode`, `dueAtUtc`, `freshness`) to `src/Pegasus.Contracts` as plain records with no EF, ASP.NET or Core types
- [ ] Add `queuePoisonCount` and `mailboxPoisonCount` to `IntakeStatusResponse` as **two separate fields**, never one sum
...
- [ ] Add the `queue_poisoned` and `received-poison:` constants to `src/Pegasus.Contracts` so neither literal is spelled twice
...
- [ ] Count queue poison from the `queue_poisoned` constant, excluding rows completed by `CompletePoisonReplay` (`EfExternalWorkStore.cs:435`, `:468`, `:499`)
- [ ] Count mailbox poison from the `received-poison:` rows of `GetEmailOperations` (`EfOperationsStore.cs:137`), projected from `ApprovedInboxPoisonMessage` (`MailboxIntake.cs:124-134`)
...
- [ ] Add the contract test asserting both poison counts appear as their own fields and are not summed
...
- [ ] Add the LocalDB integration test seeded with a failed external work item, a failed mailbox poll and one `ApprovedInboxPoisonMessage`; assert `canRetry` matches the Core projections, `limitReached` surfaces, and the two poison counts are 1 and 1
