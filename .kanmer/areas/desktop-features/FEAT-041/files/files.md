...
| `docs/adr/0108-desktop-webview2-report-rendering.md` | **Edit, Verification section only.** Cite this suite as the acceptance evidence. The file is created by [[FND-007]] (plan handle `DSK-00-07`) and owned by [[FEAT-038]] (plan handle `DSK-07-12`); the `proposed` → `accepted` frontmatter flip and the index row are **[[FEAT-038]]'s**, not this ticket's. |
