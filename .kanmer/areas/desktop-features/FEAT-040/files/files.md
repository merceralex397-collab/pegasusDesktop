...
| `src/Pegasus.Desktop.Infrastructure/Reports/OffScreenWebViewHost.cs` *(new)* | Whichever host step 2's probe chooses — a zero-size collapsed WinUI `WebView2` in a XAML root, or `CoreWebView2Controller` on a hidden HWND via `CoreWebView2Environment.CreateAsync` — behind a small internal type so the renderer does not embed the choice. This is the seam ADR-0108 records as the mitigation for the unproven-host risk. |
...
| `src/Pegasus.Desktop.Infrastructure/Pegasus.Desktop.Infrastructure.csproj` | The WebView2 / Windows App SDK reference the chosen host needs, plus `Scriban` and `PDFsharp`. Versions are inline per project today (there is no `Directory.Packages.props`; [[FND-027]] (plan handle `DSK-02-02`) introduces one), so match the versions already pinned at `src/Pegasus.Infrastructure/Pegasus.Infrastructure.csproj:24-25` — `PDFsharp` 6.2.4, `Scriban` 7.2.6 — rather than picking newer ones. |
...
| `docs/adr/0108-desktop-webview2-report-rendering.md` | The chosen off-screen host recorded in the decision's consequences — **via [[FEAT-038]]'s (plan handle `DSK-07-12`) file**, never a second ADR. |
...
| `docs/desktop/07-integrations/README.md` § 2 (WebView2 documentation facts, fetched 2026-08-23) and § 7 (risk rows) | The four documented facts (`PrintToPdfStreamAsync` returns a rewound stream; `CoreWebView2PrintSettings` covers margins/page size/backgrounds/header-footer/scale; one print per WebView at a time) and the four traps: off-screen hosting unproven, print concurrency, runtime missing, Chromium drift against a Playwright pinned to 1.61.0. |
