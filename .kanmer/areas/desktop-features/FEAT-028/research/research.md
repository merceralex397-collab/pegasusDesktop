...
(`Cases/Custody.cshtml.cs`, `inventoried`) carries
`~POST .../custody/retry`; **`PAR-19`** (`Intake/Details.cshtml.cs`,
`inventoried`) carries `~POST /api/v1/received/{id}/retry-allocation`. No row
covers mailbox-processing retry, because no web surface calls it. The matrix
...
- **The body names `CustodyRetryDecision.Decide`; the code has
  ...
  `CustodyRetryDecisionState`). The body's intent is unambiguous; the documents
  use the real names.
- **The five outcomes and the exact refusal sentences Core supplies**
  (`CustodyContracts.cs:330-370`) — each is a distinct wire case:
  `Replay` "The original custody retry request is already pending." (`:336`);
  `Conflict` "The custody retry operation key was already used for another
  request." (`:338`); `NotFound` "The case was not found." (`:343`);
  `Refused` "No matching custody work exists." (`:347`), "Only failed custody
  work can be retried." (`:355`), "Confirmed custody cannot be retried."
  (`:360`), "The case has no immutable Audit reference to store." (`:365`);
  `Conflict` again for "Another authorized retry already re-armed this custody
  work with a different operation key." (`:353`); and `Pending` "Custody retry
  queued." (`:368`). Four *different* `Refused` reasons share one outcome value,
  so the wire representation must carry the `Message`, not only the enum, or the
  operator cannot tell "already confirmed" from "no Audit reference".
- **The Automation-token refusal is already in Core.**
  `RetryCaseCustody.ExecuteAsync` (`CustodyContracts.cs:415-425`) throws
  `StaffAuthorizationException` when `request.Actor.Kind != ActorKind.Staff`
  *before* the right check. Step 8's "an Automation-client token → rejected on
  `/api/v1`" is therefore enforced twice — at the [[GWY-003]] filter and inside
  Core — and the test proves both.
- **Core's own field bounds** (`CustodyContracts.cs:435-437`): `Reason` ≤ 500,
  `OperationKey` ≤ 100, `EditLeaseToken` ≤ 200.
  `RetryExternalWork.ExecuteAsync` bounds `OperationKey` at 100
  (`RequestOperations.cs:193-198`) and rejects a negative
  `ExpectedAttemptCount` (`:190`). `RetryMailboxProcessing.ExecuteAsync`
  (`EmailOperations.cs:133-155`) bounds `MailboxId`, `ExpectedFailureCode` and
  `OperationKey` at 100 each and rejects a default `ExpectedDueAtUtc` (`:148`).
- **The audit rows already exist inside the persistence adapter, not the page.**
  `EfExternalWorkStore.RetryAsync` (`src/Pegasus.Infrastructure/Persistence/EfExternalWorkStore.cs:82`)
  writes, in one serializable transaction: a workflow history row with
  `ResultJson` (`:204-209`), a `CaseHistory` row with
  `EventType = "custody_retry_requested"` (`:210-221`) and an `ActionHistory`
  row with `EventKind = "custody_retry_requested"`,
  `CorrelationId = request.OperationKey`, `Outcome = "Succeeded"` and
  `PolicyVersion = "custody-recovery-v1"` (`:223-238`). The gateway command
  therefore inherits the audit for free — step 9 asserts the row exists, it
  does not add a writer.
- **`OperationsRetryResult` is `(bool IsReplay)`**
  (`EmailOperations.cs:114`) and is what both `RetryExternalWork` and
  `RetryMailboxProcessing` return. The `{ "isReplay": bool }` body in the body's
  step 4 matches it exactly.
- **The Razor page's exception mapping is the precedent for step 4.**
  ...
  the one step 4 maps to `urn:pegasus:problem:operation-conflict`.
- **Intake allocation throws two named exceptions**:
  `IntakeAllocationOperationConflictException`
  (`IntakeAllocation.cs:190`, "The allocation operation key was already used for
  different command details.") and `IntakeAllocationConcurrencyException`
  (`:193`, "The receipt or allocation state changed after it was loaded."), plus
  `PrincipalUnavailableException` (`:196`). Replay is decided inside
  `AllocateIntake.RetryAsync` by comparing a command hash
  (`:320-333`) — a same-key call with *different* details is a conflict, not a
  replay.
...
- **The registrations are shared, so an architecture guard must be about
  callers, not registration.** `src/Pegasus.Infrastructure/DependencyInjection.cs:217`
  registers `IRetryCaseCustody`, `:242` `RetryMailboxProcessing`, `:243`
  `RetryExternalWork` — all inside the shared composition both hosts call. A
  test asserting "the Worker does not register custody retry" would fail today
  and would be wrong. `grep -rn 'RetryCaseCustody' src/Pegasus.Worker` returns
...
- **`WorkerCompositionTests.cs` (259 lines) is the right home for that guard.**
  ...
  `IRetryCaseCustody` and `RetryCaseCustody` share an assembly, so the file
  pair to extend is known.
...
- **The problem-type catalogue is fixed.**
  ...
  `not-found` and `validation`. All five outcomes and every exception in this
  ticket map into that list; nothing new is needed.
...
- **A-07-02-3 — `RetryMailboxProcessing`'s `ExpectedFailureCode` and
  `ExpectedDueAtUtc` are exactly the `failureCode` and
  `RetryExpectedDueAtUtc` that `EmailOperationProjection` publishes**
  (`EmailOperations.cs:32`, `:34`, and `CanRetry` at `:45`). Confirmed by: an
  integration fact that a row marked `canRetry` can be retried with the values
  the same read returned. Breaks if: they diverge — the desktop would offer a
  retry that always conflicts.
- **A-07-02-4 — no Worker function will need a retry use case.** Confirmed by
  ...
  `docs/current-architecture.md:571` and FRD-05, not a test relaxation.
...
- **The custody wire shape must carry the `Message`, not only the outcome.**
  ...
  surfaces as `TempData["CaseError"] = result.Message`
  (`Cases/Custody.cshtml.cs:56`).
