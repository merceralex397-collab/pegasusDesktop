...
- Upstream evidence — **the fork board ids differ from the upstream ids in this area, and one of them collides; always cite these as written here:**
  - **upstream ENG-014 (board [[ENG-001]])** — the invented `manifest.sha256` and `provenance.json` stop being produced and the JSON is indented.
  - **upstream ENG-015 (board [[ENG-002]])** — four of the thirteen exported values are wrong: `Reference` must carry the work provider's claim number, `Inspection Address` must be a six-line block, `Vehicle Model` must carry the make, `Mileage Unit` casing.
  Both were raised from the `ap.QDOS26015` export EVA refused on 2026-08-24. **Board `ENG-001` is upstream ENG-014 and board `ENG-002` is upstream ENG-015 — the numbers do not correspond. Upstream `ENG-001` is a different ticket entirely (a post-alpha external-supplier capability, dropped and never imported), and upstream `ENG-008`, `ENG-009`, `ENG-011` and `ENG-013` have no fork tickets at all.** The `engineering-assessment` area holds exactly these two tickets.
...
- Depends on: `DSK-05-05` the case lease and version session; `DSK-07-09` the DVLA/DVSA request, accept and status endpoints with cache lifetime and provenance fields; `DSK-07-10` — owns `CaseVehicleViewModel` and `CaseVehicleView.xaml`; this slice adds the lookup-status refresh and the EVA handoff generate and download commands to them. **upstream ENG-014 (board [[ENG-001]])** and **upstream ENG-015 (board [[ENG-002]])** are already imported as fork tickets and own the mapping fix this ticket's bundle-content gate detects — they need finding on the board by those board ids, not creating.
...
- **Azure**: no write.
- **Scope boundary**: may extend `CaseVehicleViewModel` and `CaseVehicleView.xaml` in `src/Pegasus.Desktop` — [[DSK-07-10]] owns both and this slice adds members to them rather than creating its own — and may touch `src/Pegasus.Desktop.Infrastructure`, `src/Pegasus.Contracts`, the `/api/v1` vehicle and EVA groups in `src/Pegasus.Web` and the test projects. Must not reference `src/Pegasus.Infrastructure/Vehicle/` from the desktop; the desktop never calls a provider directly. Must not change `src/Pegasus.Core/Eva/EvaBundleSchema.cs`, `CaseEvaMapping` or `EvaHandoffStore.cs` — this ticket asserts the bundle's content, and **upstream ENG-014 (board [[ENG-001]])** and **upstream ENG-015 (board [[ENG-002]])** fix it.
- **Traps**: DVLA/DVSA keys stay behind the gateway (ADR-0107); the Test/UAT stack uses the replay adapter and asking for a live provider or an Azure test resource is out of bounds (L-02, ADR-0014); one normalization rule only — a second implementation is a stop condition; **upstream ENG-013** arrives via upstream sync and **upstream ENG-009** (Cazana valuation) stays backlog and must not be pulled in — neither has a fork ticket, so never write either as a board wiki-link; `Features:DesktopGateway` must be enabled in tests. One view model per screen: [[DSK-07-10]] owns `CaseVehicleViewModel`, this ticket extends it; a second view model for the same screen is a stop condition. `reuse-map.md` marks `Eva/` REUSE, so the desktop ships byte-identical output — a bundle that passes every other gate here and is still refused by EVA is the `ap.QDOS26015` failure repeating, which is why the content assertion is a gate rather than an assumption; neither **upstream ENG-014 (board [[ENG-001]])** — on upstream branch `task/eng-014-drop-manifest-indent-json` against `dev`, not in [[DSK-01-10]]'s 32-commit `main` range — nor **upstream ENG-015 (board [[ENG-002]])** — no upstream branch at all — arrives by sync, so under D-001 they exist only because the fork board holds them. **Upstream ids and fork board ids do not match, and this area collides**: board `ENG-001` is upstream ENG-014, board `ENG-002` is upstream ENG-015, while upstream `ENG-001` is an unrelated post-alpha capability that was dropped and never imported. Always write `upstream <ID> (board [[<board-id>]])`, never a bare `ENG-0nn`.
