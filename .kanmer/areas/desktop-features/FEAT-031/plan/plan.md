...
## Current-head merge, validation, and independent review — 2026-08-29

- The branch merged configured `origin/dev` at `e071d3ca43e70fd695c1f9907856d61d5b189685`; the shared gateway composition conflict was resolved by retaining both `DesktopDocumentUploadSessions` and the current OpenAPI registration/group metadata.
- Current branch head is `3e53f5e9a70eb24e1a7ee5329984f3f69b75b88b`. Worktree is clean and `git diff --check origin/dev...HEAD` passed.
...
- Focused Box broker integration tests passed 26/26. Broad non-corpus/non-browser integration passed 994/996 (2 skipped, 0 failed). Architecture tests passed 121/121.
- Independent `pegasus-desktop-reviewer` review at exact head `3e53f5e9` confirms the implemented gateway scope is review-ready but BLOCKS merge on two acceptance gates: `src/Pegasus.Infrastructure/Custody/BoxDocumentContentStore.cs:96-98` and `src/Pegasus.Infrastructure/Custody/BoxCaseCustody.cs:334-346` buffer the complete provider response before the gateway range copier, so production downloads are not genuinely streamed; and the required current-fork document-download and case-export success more than one hour after gateway revision start is not proved. Live Key Vault names-only evidence is also unavailable while cloud access remains prohibited.
- Export/evidence-gallery routes remain deliberately unexposed because the current fork has no PLAT-041 O(1)+N implementation and measurement. No Box URL, token, object id, or provider secret is exposed in the changed API/desktop contracts.
- Next action is to route the permitted provider-streaming change and obtain current-fork token-age evidence; no merge or Done claim is made until both required gates are satisfied.

## CI repair — 2026-08-30

- Exact-head CI run `33272593232` failed only in unit for the OpenAPI snapshot and command-coverage guard. The failure named five live command routes: POST `/api/v1/cases/{caseId}/documents/upload-session`, DELETE `/api/v1/cases/{caseId}/documents/{occurrenceId}`, POST `/api/v1/cases/{caseId}/third-party-vehicle-evidence/confirm`, PUT `/api/v1/upload-sessions/{sessionId}`, and POST `/api/v1/upload-sessions/{sessionId}/complete`.
- Corrected `eng/api/Export-OpenApiDocument.ps1` so its PowerShell normalizer preserves singleton OpenAPI arrays; regenerated `openapi/pegasus-v1.json` with the prescribed exporter.
- Added one explicit route row for each of the five command endpoints in `tests/Pegasus.Api.ContractTests/CommandCoverage/CommandCoverageTable.cs`. The SQL-free generic contract host has no case/Core command doubles, so these rows are explicit placeholders; endpoint behavior remains covered by the existing Box broker LocalDB/Ef integration suite and is not claimed by the placeholders.
- Local validation: Release solution build passed with 0 warnings/errors; the 24-test `Category=Contract` suite passed with 0 failures/skips; `git diff --check` passed.
- This repair only addresses the CI registration/snapshot defect. It does not resolve the independent-review blockers for provider-response buffering, >1-hour token-age proof, PLAT-041 call-budget proof, or live Key Vault evidence.
