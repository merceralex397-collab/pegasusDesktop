...
The exact-head unit failure from run `33272593232` was reproduced locally. The OpenAPI exporter was collapsing singleton arrays during PowerShell function return; its list preservation is corrected, and the snapshot was regenerated. The command-coverage guard now has explicit rows for all five live Box command routes. Release solution build passed with 0 warnings/errors and all 24 `Category=Contract` tests passed. The repair does not change the ticket's substantive acceptance disposition: provider-response buffering, current-fork token-age proof, PLAT-041 call-budget implementation/measurement, and live Key Vault evidence remain open; no merge or Done claim is made.
