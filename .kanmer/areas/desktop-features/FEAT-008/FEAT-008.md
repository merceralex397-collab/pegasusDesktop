...
updated: '2026-08-24T08:51:18.373Z'
---
...
8. Implement the lease-lost path: the editor becomes read-only immediately, the current holder is named, and the operator is offered re-claim (which re-queries first) rather than a silent re-acquire.
...
11. Add view-model tests in `tests/Pegasus.Desktop.ViewModelTests` covering each problem type mapped to its state, the comparison producing only differing fields, refusal to retry a non-idempotent command without a fresh key, and re-query on timeout.
...
- [ ] A lost lease immediately makes the editor read-only and names the holder.
...
- [ ] `dotnet test ./tests/Pegasus.Desktop.ViewModelTests/Pegasus.Desktop.ViewModelTests.csproj --configuration Release --no-build` — expected: state mapping, comparison, retry-rule and re-query facts pass.
...
- **Azure**: no write.
- **Scope boundary**: may touch `src/Pegasus.Desktop`, `src/Pegasus.Desktop.Infrastructure`, `src/Pegasus.Contracts`, the `/api/v1` problem-details mapping in `src/Pegasus.Web` and the test projects. Must not modify `CaseMutationPageModel.cs` or any Razor page.
- **Traps**: do not reproduce retained proposed values — the desktop compares against a fresh server read instead; banned words include `lease`, so operator copy must use settled vocabulary; a blind retry of a non-idempotent command is a defect, not a convenience; upstream KANMER-005 (exclusive leases between staff and Automation Actors) is a gateway rule absorbed here — confirm it is implemented on the endpoint before claiming parity; `Features:DesktopGateway` must be enabled in tests.
