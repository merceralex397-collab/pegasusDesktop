...
updated: '2026-08-24T11:05:49.695Z'
---
...
- Upstream evidence: `DOCS-001` (readiness and idempotency are Core-owned and fail closed on missing, unaccepted or ambiguous required data; its own research records that no durable report request/version aggregate, payload-identity, generation-state, attempt, lease or failure tables exist) and `TICK-208` (Core carries one `ReportApprovalId` and one `ReportSentEvidenceId` per case, so a corrected version currently risks replacing the earlier pointer; its plan makes the durable authority an append-only issued-version-to-Sent-evidence association with reasoned history)
...
- Depends on: `DSK-07-05` the custody upload path; `DSK-07-14` the desktop renderer producing the artifact and its provenance; `TICK-208` as recreated on the fork board by [[DSK-01-09]] (unconditionally — see [[DSK-07-17]] step 7) — the append-only issued-version to Sent-evidence ledger, without which step 11's separately shown issued versions are not implementable
