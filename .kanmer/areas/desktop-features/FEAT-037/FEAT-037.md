...
- **Traps**: scope creep into MAIL-12/13/17/19 is out of conversion scope (proposal § 13.11) — only the seam is built; ADR-0106 keeps the mail service credential central and a Graph credential in the desktop package is a defect; `unknown` must never collapse into success; a second copy of the outbound state vocabulary is duplication; carrying the classification is not carrying a query lifecycle — creating, replying to or resolving a query stays with upstream CASE-002; a policy key or decision version on the wire is an upstream PLAT-015 breach — **note the collisions: upstream PLAT-015 has no fork ticket and the board's `PLAT-015` is `DSK-10-15`; the board's `CASE-002` is upstream CASE-022 (public upload links), not the upstream CASE-002 named above; neither upstream CASE-002 nor upstream CASE-009 was imported** (`HZN-001` group document `board-conventions.md` § Upstream ids versus board ids holds the join table); a new table for idempotency bookkeeping would need a runtime-role `Grant*` migration (`scripts/Test-MigrationGrants.ps1`) — reuse the existing operation-key mechanics instead.
