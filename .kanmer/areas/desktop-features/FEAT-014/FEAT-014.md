...
- Depends on: `DSK-05-05` the case lease and version session; `DSK-07-05` the Box broker endpoints; `DSK-07-07` the spike deciding direct transfer versus gateway streaming; `DSK-07-06` — owns `CaseDocumentsViewModel`, `CaseDocumentsView.xaml` and `TransferQueueService`; this slice adds the export, custody-retry and permission-checked removal commands to them
