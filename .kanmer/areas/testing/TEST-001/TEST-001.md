...
updated: '2026-08-24T08:51:05.947Z'
---
...
5. Add `OpenApiSnapshotTests.cs`: resolve the OpenAPI document from the running factory, serialize it with stable ordering and indentation, and compare it byte-for-byte with the committed snapshot `openapi/pegasus-v1.json` (committed by [[DSK-03-04]]). When the file is absent, write the produced document to `openapi/pegasus-v1.json.actual` and fail with a message naming that path and the command to promote it. Done when an added endpoint makes exactly this test red.
