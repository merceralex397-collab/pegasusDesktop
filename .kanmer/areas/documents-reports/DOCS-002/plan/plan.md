...
## Current fork and cloud boundary amendment — 2026-08-26

All implementation work remains in this fork from `origin/dev`; no upstream synchronization is required or permitted. The live Box verification remains a later operator-owned external write inside the approved disposable subtree. It is not performed or claimed here because current operator policy prohibits cloud/Box writes until the full refactor is complete. The per-attachment interpretation remains an explicit open product decision; do not infer that the retained `.eml` or separate files satisfies DOC-02.
