...
- **Reuse:** the change reuses `StaffActorFactory`, `StaffAuthorization`, `DesktopGatewayCorrelation`, `DesktopGatewayProblems`, `ISecurityEventWriter`, and the existing gateway resolver seam. The test uses the existing `IntakeWebApplicationFactory` and integration authentication handler rather than adding a new host or fixture framework.
- **Simplification:** the claims-to-actor call remains in one new accessor, the existing resolver delegates to it, and the right filter contains only actor retrieval plus the Core authorization call. No alternate policy engine, cache, compatibility path, or production test endpoint was added; the extra route exists only in the test startup filter.
- **Efficiency:** account enabled/security-stamp validation remains an uncached per-request lookup as required by GWY-021. The actor is resolved once and handed to the right filter through the existing request item context; no handler re-resolution or extra database query was introduced.
- **Altitude:** business-state rules remain in Core. Gateway code performs transport/session/actor boundary work, and integration tests exercise the composed `/api/v1` group with correlation-linked security-event evidence.
- **Findings/dispositions:** no behaviour-preserving simplification was identified that would reduce scope or improve clarity without weakening the required evidence. The disabled-account, invalid-stamp, and absolute-expiry denials share the existing token-event writer; Automation/non-staff denials share the new accessor writer. Independent review remains required before the PR.

## Review reconciliation — 2026-08-30

The independent reviewer identified that the two permanent-refusal data rows named only Administrator and User even though the checklist requires all three staff roles. The test keeps the required 27-fact count and now makes both `ExecuteSystemWork` and `SubmitRequestUpload` refusal facts issue requests as Administrator, Engineer, and User, asserting 403 for each role; no actor-factory widening is needed.

The reviewer also identified the claim-shape wording difference: the landed GWY-021 bearer principal emits OpenIddict `Claims.Subject` and `Claims.Role`, while the test/cookie-shaped principal can expose `ClaimTypes.NameIdentifier` and `ClaimTypes.Role`. `StaffActorAccessor` therefore treats the OpenIddict subject as authoritative, falls back to `ClaimTypes.NameIdentifier` for the supported test/cookie shape, and unions the two role claim types before passing them once to `StaffActorFactory`. This is fail-closed on divergent/unknown role values and is the intentional seam required by the already-landed bearer implementation; it does not add another parser or policy owner.

The reviewer confirmed the four simplification lenses and found no merge-blocking production issue. The dated simplification pass above is the current disposition; the earlier placeholder “Not yet run” text is superseded by that section. The remaining gate is the post-implementation report with exact validation output and the subsequent PR review/CI/merge sequence.
