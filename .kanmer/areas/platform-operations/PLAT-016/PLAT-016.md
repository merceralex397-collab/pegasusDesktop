...
- Plan row: `docs/desktop/10-security-observability-performance/README.md` § 5 — `DSK-10-16` (marked ⚠)
- Plan detail: same file § 3 ("⚠ Raising the Log Analytics daily cap (PLAT-036) is an Azure write, conditional on exact-target approval after the volume measurement" and "⚠ A new alert rule … is a Bicep change, conditional on approval"), § 7 ("Alert/quota changes made casually")
- Proposal: `docs/desktop/Pegasus_Native_Desktop_Design_Proposal.md` § 18.2 Central telemetry `:1215-1227`; § 19 Azure service disposition `:1245-1305`
- Repository evidence:
  - `infra/modules/platform.bicep:46-55` — the Log Analytics workspace resource (the cap is a portal/workspace setting, `workspaceCapping.dataIngestionStatus: RespectQuota`, not currently expressed in this template — confirm before editing)
  - `infra/modules/platform.bicep:56-67` — the App Insights component; `:68-...` the action group `${prefix}-operations`
  - `infra/modules/platform.bicep:576-616` — `pegasus-prod-web-http5xx` (metric alert, `if (webActivationApproved)`); `:617-689` — `pegasus-prod-application-exceptions` (scheduled query rule) — the two shapes a third rule would copy
  - `docs/desktop/11-azure-disposition/README.md:156` (`DSK-11-09` cap decision input), `:169-170` (retain rows with the two ⚠ markers), `:189-190` (the conditional-write register rows: "Log Analytics daily cap change" and "Additional alert rule (blocked-client spike)")
  - `docs/runbook.md:776-790` — the live-operation approval matrix, row "Change or use an Azure service (write/mutation/cost)"
  - `docs/desktop/10-security-observability-performance/telemetry-volume.md` — the measurement this decision rests on ([[DSK-10-14]])
- Binding decisions:
  - **Azure rule** — reads are free; every write is marked ⚠, needs exact-target approval, and is mirrored in plan 11. Nothing is deprovisioned before cutover.
  - **ADR-0109** — the diagnostics bundle is the primary support tool; raising the cap is an improvement, not a prerequisite.
  - **C-01** — cost is a live constraint; a cap increase is a recurring bill and must be priced before it is proposed.
- Depends on: `DSK-10-14` — the desktop-era volume measurement and the telemetry dimensions the alert would fire on.
...
- [ ] A decision file exists with the measured evidence, the options including volume reduction, and a monthly cost figure derived from a real per-GB price.
- [ ] Each of the two items has an explicit recorded decision with the exact target, the approver and the date — including "not approved", which is a complete outcome.
- [ ] No Azure resource is changed without that recorded exact-target approval.
- [ ] If the alert rule is approved: `platform.bicep` carries it behind an activation guard, `Test-AzureDeploymentPlan.ps1 -Mode Local` passes, and an `azure-validate` what-if shows exactly one added resource.
- [ ] If the cap is changed: before/after values, approver, date and the rollback are recorded.
- [ ] The plan-11 conditional-write register rows are updated to match.
...
Tier 9 — Security/observability. Here that obliges an evidenced alerting/retention decision with its cost, and — where a change is made — a what-if proving the exact delta before deployment.

## Documentation changes

- `docs/desktop/10-security-observability-performance/telemetry-decision.md` — new file with the evidence, options, cost and decision.
- `docs/desktop/11-azure-disposition/README.md` — conditional-write register rows `:189-190` updated with the outcome.
- `docs/operations.md` — alert and quota state.
- `docs/current-architecture.md` — PLAT-034 / PLAT-036 status.
