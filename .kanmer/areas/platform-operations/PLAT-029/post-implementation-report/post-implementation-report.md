...
## Repeated clean Start validation — 2026-08-26

With the refreshed initialization manifest still matching exact head `7d761ed6dbe66fd274bac3701618980499bf0a47`, the exact prescribed Start command was run again and failed for run `77ce3201aa7b4165b8bbbe9712b177ba` at `scripts/Invoke-LocalDevelopment.ps1:1482`; `Start-OwnedLauncher` line 754 passed an empty `Process.Path` to `GetFullPath`. This is a second clean reproduction after `ff3396ad098a49fd85c6a095b13f30b9`, not a stale initialization failure. The run was stopped with the repository Stop command (exit 0). Smoke and the operator-visible journey remain unproven; the launcher correction belongs to TEST-017 and is outside this ticket's source scope.
