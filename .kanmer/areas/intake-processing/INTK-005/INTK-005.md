...
updated: '2026-08-25T20:49:27.196Z'
---
...
1. Orient. Read the verbatim upstream body above and `coverage-decision.md` § Import list row for upstream `INTK-031`. Call `get_doc_gates <this ticket id>`, then `take_ticket` with branch `task/upstream-intk-031-issuer-identification` and worktree `../pegasus-worktrees/upstream-intk-031-issuer-identification` from `origin/dev`.
...
8. Specify the abstention contract precisely enough for the imported `upstream:INTK-032` to build on: what "no issuer matched" produces, what "issuer matched but outcome unreadable" produces, and how those differ from today's `null`. Do **not** design the operator-visible outcome here — that is upstream INTK-032's (board [[INTK-006]]), and its operator has deferred it.
...
- [ ] The only edit this ticket makes to `upstream-kanmer-carryover.md` is the upstream `INTK-031` row annotation. The § Disposition categories sentence restricting `unchanged-backlog` to rows that **have** a `docs/capabilities.md` row is **cited, not written** — [[DSK-01-09]] step 15(e) is its single owner.
