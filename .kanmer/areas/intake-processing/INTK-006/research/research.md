...
- The branch is `task/upstream-intk-032-unreadable-report-fallback`, based on `origin/dev` at `fff7e14178f1be6e3d4f2fbc5a5401799ba69409`; the own worktree is `../pegasus-worktrees/upstream-intk-032-unreadable-report-fallback`. It is clean and no product code has been changed.
- `src/Pegasus.Core/Intake/DirectProviders/Qdos/QdosMailClassificationPolicy.cs:167-178` calls `EvaluateStandaloneAuditReport` only for `CaseType.Audit` and stores its nullable result in `MailClassificationResult.StandaloneAuditReport`. The evaluator groups document/PDF attachment fragments by asset, requires at least two attachments and exactly one audit-notification attachment (`:181-205`), and returns null when no report has exactly one recognised outcome (`:206-215`). `ContainsRepairable` and `ContainsTotalLoss` are literal-plus-negation checks (`:224-230`).
- `src/Pegasus.Core/Intake/ProcessIntake.cs:189-199` currently treats a classified Audit with null `StandaloneAuditReport` as generic `NeedsSorting`, clears the instruction draft and missing fields, and uses the wording `A standalone Audit instruction requires one attached original report stating Repairable or Total loss.` This conflates an ordinary non-audit classification shape with an unreadable/ambiguous report. `ProcessIntake.cs:342-365` returns from automatic evidence recording when the evaluation is null; it cannot retain a report assessment that was not produced.
- `src/Pegasus.Core/Intake/AcceptIntake.cs:60-77` refuses Audit acceptance when `StandaloneAuditEvidenceId` is null and rejects an empty evidence id. `src/Pegasus.Core/Cases/CaseContracts.cs:93-108` maps `Repairable` to immutable `a.` and `TotalLoss` to immutable `ap.`; therefore an invented result cannot be corrected by reopening or changing a case reference.
- The ticket's old reference to a future first one-way upstream sync is superseded for this fork: repository instructions say no upstream syncs and all work remains in this repo. INTK-005 is already done on this board; its research is read-only provenance. Implementation, when authorized by the operator decision, must use the current fork `origin/dev` and must not wait for or perform upstream synchronization.
